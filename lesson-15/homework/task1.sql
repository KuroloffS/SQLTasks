...
DECLARE @identifier_name NVARCHAR(128);
DECLARE @properties NVARCHAR(MAX);
DECLARE @sql NVARCHAR(MAX);

-- Assume all contacts use the same identifier type (e.g., email)
SELECT TOP 1 @identifier_name = identifier_name FROM Contacts;

-- Generate property list as ('column', value) for unpivoting
SELECT @properties = STRING_AGG(
    CONCAT('(''', COLUMN_NAME, ''', ', QUOTENAME(COLUMN_NAME), ')'), 
    ', '
)
...
SET @sql = N'
SELECT 
    identifier_value AS ' + QUOTENAME(@identifier_name) + ',
    (
        SELECT 
            prop.[property],
            prop.[value]
        FROM (VALUES ' + @properties + '
        ) AS prop([property], [value])
        FOR JSON PATH
    ) AS properties
FROM Contacts
FOR JSON PATH;';
